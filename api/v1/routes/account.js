...
router.get('/', (req, res) => {
  ...
  dbModule.open(pool, (con) => {
    con.query(
      'SELECT salt FROM students WHERE id=?',
      [id],
      function (err, result) {
        if (err) {
          ...
          const hash = crypto
            .createHash('sha512')
            .update(password.concat(result[0].salt))
            .digest('hex');

          con.query(
            'SELECT * FROM students WHERE id=? AND hash=?',
            [id, hash],
            function (err, result) {
              if (err) {
                console.log('DB communication failed: ', err);
    ...
    con.query(
      ...
      function (err, result) {
        if (err) {
          ...
          const salt = crypto
            .createHash('sha512')
            .update(crypto.randomBytes(20).toString('base64'))
            .digest('hex');
