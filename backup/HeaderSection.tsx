...
const HeaderSection = () => {
  ...
  const isLogin = useSelector((state: RootState) => state.user.isAuth);
  ...
  return (
    <>
      <section className="header-section">
        <h1 className="mb-0">
          {isLogin ? (
            ...
            <>
              <a
                href="#javascript"
                onClick={() => openModal(modals.loginModal, { show: true })}
        ...
        <h1 className="mb-0" onClick={handleShow} aria-hidden="true">
          <i className="fa-solid fa-bell" style={{ color: 'dimgray' }} />
        </h1>
      </section>
        ...
        <Offcanvas.Body style={{ boxSizing: 'border-box' }}>
          <ToastContainer style={{ position: 'relative' }}>
            <Toast style={{ width: '100%' }}>
              ...
              <Toast.Body>
                {dayjs().get('M')}월 {dayjs().get('D')}일 오후 2시부터 학생회관
                1층에서 COVID-19 자가진단 키트를 제공해 드립니다. 한정 수량이니
                필요한 학생은 시간에 맞춰 수령 바랍니다.
              </Toast.Body>
            </Toast>
            <Toast>
              <Toast.Header closeButton={false}>
